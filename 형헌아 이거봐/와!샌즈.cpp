	...
	for(h=0; h<50; h++){
		for(w=0; w<190; w++){
			...
			if(temp == '0'){
				...
				*x = w; // 낚싯찌  
				*y = h; 
				printf(BG_COLOR(0,0,0) FG_COLOR(150,150,150)"J" RESET);
			}else if(temp == 'O'){
...
void move(int* x, int* y, int _x, int _y){
	...
	if (mapObject == '0'){
		...
		*y += _y;
		...
		gotoxy(*x, *y);
		printf(BG_COLOR(204,105,5) FG_COLOR(255,255,255)"/" RESET);//black
	
		gotoxy(*x + _x, *y + _y);
		printf(BG_COLOR(0,112,192) FG_COLOR(150,150,150) "J" RESET);
...
void showstock()
{
	...
	string moneyname[3] = {"stock1", "stock2", "stock3"};
	int num = -1, where = -1;
	string inout;
	gotoxy(2, 49);
	cout << "매수 매도 선택: ";
	...
	if(inout == "매도")
	{
		if(stock_index != -1 && stock_num != -1)
		{
			...
			/*
				지금은 한종목만을 가지고 
				n개를 사는걸로해놓았음
				여기에는 가지고 있는 것중 몇개를 팔입력받아서 그만큼 팔고
				mymoney에 그만큼에 돈을추가함 
			*/
		}
		else
	...
	/*
		입력받은 문자나 수가 없으면 다시 입력하도록 알려줘야함
		+ showstock함수를 호출할때마다 값이 증가하거나 감소하도록하고
			주식을 여러게 사도 문제 없게 해야함
	...
	*/
	...
	bankMoney[where] *= sik[random];
	
	gotoxy(129, 53);
	cout << moneyname[where] << " " << bankMoney[where];
}
...
int gamemenuDraw(){ //자동차 미리보기  
	...
	Sleep(150);
	gotoxy(30,6); 
	printf("너에게 말해주지 않았었지만 이 할아비는 사실 김주헌그룹의 회장이었단다.\n");
	Sleep(150);
	gotoxy(30,8);
	printf("너는 나의 유일한 후계자이기에 주헌그룹은 너에게 상속될 예정이란다.\n");
	Sleep(150);
	gotoxy(30,10);
	printf("그러나 그 전에, 너가 과연 김주헌그룹을 물려받을 수 있는 그릇이 될지 시험해보기 위해\n");
	Sleep(150);
	gotoxy(30,12);
	...
	Sleep(150);
	gotoxy(30,16);
	printf("하지만 1억을 갚지 못한다면 1억의 빚은 우리 김주헌그룹에서 갚아주겠지만, 회사를 물려주지는 않을 것이란다.\n");
	Sleep(150);
	gotoxy(30,18);
	printf("네가 꼭 빚을 갚고 우리의 김주헌그룹을 물려받을 수 있기를 바란다...\n");
	Sleep(150);
	gotoxy(30,20);
	printf("-할아버지가\n");
	Sleep(150); 
